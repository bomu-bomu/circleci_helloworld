...
const identity_ial = process.env.IDENTITY_IAL || 3; // For IDP create identity
// RP create request
...
const timeoutWaitStatus = parseInt(process.env.TIMEOUT_STATUS_FROM_IDP || '10000');
...
AfterAll(() => {
  setTimeout(() => {
    try {
      const testResult = exec(`sh ${path.join(__dirname, '..', '..', 'scripts', 'test-result-to-junit-xml.sh')} test-result-authen`);
