...
// IDP create identity
const namespace = process.env.NS || 'cid';
const identifier = process.env.ID || '1234';
const identity_ial = process.env.IDENTITY_IAL || '2.3';
// IDP create response
...
Before('@CreateResponse', (data, callback) => {
  if (req_body) {
...
Before('@CreateIdentity', (data, callback) => {
  if (req_body) {
...
Before('@ResponseFromConsentOnboard', (data, callback) => {
  if (req_body) {
...
Given('IDP client making a request for create response', function (data, callback) {
  if (this.RequestFromIdpPlatform) {
    const sid = `${this.RequestFromIdpPlatform.namespace}-${this.RequestFromIdpPlatform.identifier}`;
    const dataRequest = JSON.parse(data);
    this.requestBody = {
      ...dataRequest,
...
Then('The response for create new identity', function (callback) {
  if (this.actualResponse) {
    ...
    const sid = `${RequestBodyCreateIdentity.namespace}-${RequestBodyCreateIdentity.identifier}`;
