...
let timeoutWaitStatus = parseInt(process.env.TIMEOUT_WAIT_STATUS_FROM_IDP || '20000');
//Timed out for wait reponse onboard request
let timeoutWaitResponseOnboardRequest = parseInt(process.env.TIMEOUT_WAIT_ONBOARD_REQUEST || '20000');
//Timed out for RP wait data from AS
let timeoutWaitDataFromAS = parseInt(process.env.TIMEOUT_WAIT_DATA_FROM_AS || '20000');
...
AfterAll(function(){
  setTimeout(function(){
    try{
    let testResult = exec(`sh ${path.join(__dirname,"..","..","scripts","test-result-to-junit-xml.sh")} test-result-dataRequest`);
